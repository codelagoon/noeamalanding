...
const strategies = [
  ...
  { name: 'Binary High (Strict)',approvalRate: 52, defaultRate: 2.9, fill: '#FF5C6C', key: 'strict'  },
  { name: 'Binary Low (Lenient)',approvalRate: 74, defaultRate: 5.2, fill: '#F2C14E', key: 'lenient' },
  { name: 'Noema Binary Medium', approvalRate: 67, defaultRate: 3.2, fill: '#6EA8FE', key: 'medium'  },
  { name: 'Bayesian Optimal',    approvalRate: 71, defaultRate: 3.1, fill: '#5FD1C4', key: 'optimal' },
];
...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: { payload: typeof strategies[0] }[] }) {
  ...
  return (
    <div className="bg-navy-900 border border-navy-700 rounded-xl px-4 py-3 shadow-xl shadow-black/50">
      ...
      <p className="font-mono text-[10px] text-[#A7B0C0]">Approval rate: <span className="text-white font-semibold">{d.approvalRate}%</span></p>
      <p className="font-mono text-[10px] text-[#A7B0C0]">Default rate: <span className="text-white font-semibold">{d.defaultRate}%</span></p>
      {d.key === 'medium' && (
        <p className="font-mono text-[9px] text-accent mt-1.5 border-t border-navy-700 pt-1.5">
    ...
    <section className="px-8 py-24 border-t border-navy-700">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="font-mono text-sm text-[#A7B0C0] mb-10 max-w-xl leading-relaxed">
          ...
          routed to human-in-the-loop review — closes <span className="text-accent font-semibold">40% of the performance gap</span> between
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
          {[
            { value: 40, suffix: '%', label: 'decisioning gap closed vs. human-only', color: 'text-accent', source: 'PoC benchmark' },
            { value: 67, suffix: '%', label: 'approval rate at Binary Medium threshold', color: 'text-[#F5F7FA]', source: 'vs 58% human-only' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-xl border border-navy-700 bg-navy-900 p-6">
            <p className="font-mono text-[10px] text-[#A7B0C0] uppercase tracking-widest font-semibold mb-1">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <ScatterChart margin={{ top: 12, right: 20, left: -8, bottom: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#131926" />
                <XAxis
                  ...
                  label={{ value: 'Approval Rate (%)', position: 'insideBottom', offset: -12, fontFamily: 'monospace', fontSize: 9, fill: '#6E788A' }}
                />
                <YAxis
                  ...
                  tickFormatter={(v: number) => `${v}%`}
                  width={36}
                  label={{ value: 'Default Rate (%)', angle: -90, position: 'insideLeft', offset: 18, fontFamily: 'monospace', fontSize: 9, fill: '#6E788A' }}
                />
                <Tooltip content={<CustomTooltip />} cursor={{ stroke: '#1E2635', strokeWidth: 1 }} />
            ...
            <div className="flex flex-wrap gap-4 mt-2">
              {strategies.map((s) => (
                <div key={s.key} className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-[9px] text-[#6E788A]">{s.name}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-navy-700 bg-navy-900 p-6 flex flex-col gap-4">
            <div>
              ...
              <p className="font-mono text-sm text-[#F5F7FA] font-semibold">AI score 34–66: Refer, not deny</p>
            </div>

            <div className="flex-1 flex flex-col gap-3">
              {[
                { score: '0–33',   label: 'Deny',   color: 'bg-[#FF5C6C]', text: 'text-[#FF5C6C]', detail: 'Clear denial signal. SHAP narratives generated automatically.' },
                ...
                <div key={score} className="flex items-start gap-4 p-4 rounded-xl border border-navy-700 bg-navy-850">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="font-mono text-[10px] text-[#6E788A]">Score {score}</span>
                    </div>
                    <p className="font-mono text-[10px] text-[#A7B0C0] leading-relaxed">{detail}</p>
                  </div>
                </div>
              ))}
            </div>

            <div className="p-3 rounded-xl border border-accent/15 bg-accent/5">
              <p className="font-mono text-[10px] text-accent leading-relaxed">
