...
const costData = [
  { label: 'Build In-House', months: 15, cost: 100, fill: '#A7B0C0' },
  { label: 'SaaS-Only',     months: 3,  cost: 55,  fill: '#94A3B8' },
  { label: 'Noema Hybrid',  months: 1,  cost: 30,  fill: '#6EA8FE' },
];
    ...
    <section className="px-8 py-24 border-t border-[#1E2635]">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
          <div className="md:col-span-1 rounded-xl border border-accent/40 bg-accent/10 p-6 flex flex-col justify-between">
            ...
            <p className="font-mono text-xs text-[#A7B0C0] leading-relaxed">
          ...
          <div className="md:col-span-2 rounded-xl border border-[#1E2635] bg-[#0A0D12] p-5">
            <p className="font-mono text-[10px] uppercase tracking-wider text-[#A7B0C0] mb-4">
                  ...
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div className="bg-[#06070A] border border-[#1E2635] rounded-lg px-3 py-2">
                        <p className="font-mono text-xs font-semibold text-[#F5F7FA]">{d.label}</p>
                        <p className="font-mono text-[10px] text-[#A7B0C0]">Cost index: {d.cost}</p>
                        <p className="font-mono text-[10px] text-[#A7B0C0]">Time to deploy: ~{d.months}mo</p>
                      </div>
                    );
                  }}
                />
                <ReferenceLine y={30} stroke="#6EA8FE" strokeDasharray="4 3" strokeWidth={1} />
        ...
        <div className="rounded-xl border border-[#1E2635] overflow-hidden">
          <div className="grid grid-cols-4 bg-[#0A0D12] border-b border-[#1E2635] px-5 py-3">
            <p className="font-mono text-[10px] uppercase tracking-wider text-[#A7B0C0]">Dimension</p>
            <p className="font-mono text-[10px] uppercase tracking-wider text-[#A7B0C0]">Build in-house</p>
            <p className="font-mono text-[10px] uppercase tracking-wider text-[#A7B0C0]">SaaS-only</p>
            <p className="font-mono text-[10px] uppercase tracking-wider text-accent">Noema Hybrid ✓</p>
          </div>
          {rows.map((row, i) => (
            <div
              key={row.dimension}
              className={`grid grid-cols-4 px-5 py-3.5 gap-4 ${i < rows.length - 1 ? 'border-b border-[#1E2635]' : ''} hover:bg-[#0A0D12]/60 transition-colors`}
            >
              <p className="font-mono text-xs text-[#A7B0C0] font-semibold">{row.dimension}</p>
              <p className="font-mono text-xs text-[#A7B0C0]">{row.inHouse}</p>
              <p className="font-mono text-xs text-[#A7B0C0]">{row.saas}</p>
              <p className="font-mono text-xs text-accent font-semibold">{row.hybrid}</p>
            </div>
          ))}
        </div>

        <p className="mt-5 font-mono text-[10px] text-[#A7B0C0]">
