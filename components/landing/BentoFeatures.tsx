...
const proxyData = [
  { variable: 'credit_score', score: 0.81, fill: '#FF5C6C', risk: true },
  { variable: 'income',       score: 0.63, fill: '#FF5C6C', risk: true },
  { variable: 'dti',          score: 0.29, fill: '#F2C14E', risk: false },
];
...
const ldaData = [
  { name: 'Cash-flow + Rental History', reduction: 4.1,  source: 'Urban Institute 2021', fill: '#5FD1C4' },
  { name: 'Threshold Recalibration',    reduction: 3.3,  source: 'Fuster et al. 2022',   fill: '#5FD1C4' },
  { name: 'Drop High-Proxy Variables',  reduction: 2.8,  source: 'Chouldechova 2017',    fill: '#F2C14E' },
];
...
function DarkTooltip({ active, payload, label }: {
  ...
  return (
    <div className="bg-[#06070A] border border-[#1E2635] rounded-lg px-3 py-2">
      {label && <p className="font-mono text-[10px] text-[#A7B0C0] mb-1">{label}</p>}
      {payload.map((p, i) => (
        <p key={i} className="font-mono text-xs font-semibold text-[#F5F7FA]">
  ...
  return (
    <div className="relative w-full" style={{ height: 160 }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center" style={{ paddingBottom: 18 }}>
        ...
        <span className="font-mono text-[9px] text-[#A7B0C0] tracking-wider uppercase mt-0.5">DIR</span>
      </div>

      {/* scale labels */}
      <span className="absolute font-mono text-[9px] text-[#A7B0C0]" style={{ left: '8%', bottom: 4 }}>0.0</span>
      <span className="absolute font-mono text-[9px] text-[#A7B0C0]" style={{ right: '8%', bottom: 4 }}>1.0</span>
    ...
    <ResponsiveContainer width="100%" height={120}>
      <AreaChart data={driftData} margin={{ top: 8, right: 4, left: -18, bottom: 0 }}>
        ...
        <ReferenceLine
          ...
          strokeDasharray="5 3"
          strokeWidth={1.5}
          label={{ value: '0.80', position: 'right', fontFamily: 'monospace', fontSize: 9, fill: '#FF5C6C' }}
        />
        <Area
          type="monotone"
          dataKey="dir"
      ...
      >
        ...
        <Tooltip
          content={({ active, payload }) => {
            ...
            return (
              <div className="bg-[#06070A] border border-[#1E2635] rounded-lg px-3 py-2">
                <p className="font-mono text-xs font-semibold text-[#F5F7FA]">{d.variable}</p>
                <p className="font-mono text-[10px] text-[#A7B0C0]">score: {d.score.toFixed(2)}</p>
      ...
      >
          ...
          content={({ active, payload }) => {
            ...
            return (
              <div className="bg-[#06070A] border border-[#1E2635] rounded-lg px-3 py-2">
                ...
                <p className="font-mono text-[10px] text-[#6EA8FE]">−{d.reduction}pp disparity</p>
                <p className="font-mono text-[9px] text-[#A7B0C0]">{d.source}</p>
              </div>
            );
          }}
        />
        <Bar dataKey="reduction" radius={[0, 4, 4, 0]}>
          ...
          <LabelList
            ...
            position="right"
            formatter={(v) => `−${v}pp`}
            style={{ fontFamily: 'monospace', fontSize: 10, fill: '#A7B0C0' }}
          />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
...
function BentoCard({ className = '', children }: BentoCardProps) {
  return (
    <div className={`bento-card p-6 group hover:border-[#6EA8FE]/50 transition-colors duration-300 ${className}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <section className="px-8 py-24 border-t border-[#1E2635]">
      <div className="max-w-6xl mx-auto">
        <p className="font-mono text-[11px] tracking-[0.15em] uppercase text-[#A7B0C0] mb-4">Platform Capabilities</p>
        <h2 className="font-serif text-[clamp(28px,4vw,44px)] leading-[1.15] text-[#F5F7FA] mb-12 max-w-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 auto-rows-auto">
          ...
          <BentoCard className="md:row-span-2 flex flex-col">
            <div className="flex items-center gap-2 mb-3">
              <Zap size={16} className="text-[#6EA8FE]" />
              <p className="font-mono text-xs font-semibold text-[#A7B0C0] uppercase tracking-wider">Real-time DIR Gauge</p>
            </div>
            <p className="font-mono text-sm text-[#F5F7FA] font-semibold mb-1">Disparate Impact Ratio</p>
            <p className="font-mono text-xs text-[#A7B0C0] mb-4 leading-relaxed">
            ...
            <div className="mt-4 pt-4 border-t border-[#1E2635] space-y-1">
              <div className="flex justify-between font-mono text-[10px]">
                <span className="text-[#A7B0C0]">Reference group approval rate</span>
                <span className="text-[#F5F7FA] font-semibold">68.4%</span>
              </div>
              <div className="flex justify-between font-mono text-[10px]">
                <span className="text-[#A7B0C0]">Comparison group approval rate</span>
                <span className="text-red-400 font-semibold">52.7%</span>
              </div>
              <div className="flex justify-between font-mono text-[10px]">
                <span className="text-[#A7B0C0]">Bartlett et al. (2022) basis</span>
                <span className="text-[#A7B0C0]">3.6M loans</span>
              </div>
            </div>
          ...
          <BentoCard className="md:col-span-2">
            <div className="flex items-center gap-2 mb-2">
              <TrendingDown size={16} className="text-[#F2C14E]" />
              <p className="font-mono text-xs font-semibold text-[#A7B0C0] uppercase tracking-wider">Fairness Drift Detection</p>
            </div>
            <p className="font-mono text-sm text-[#F5F7FA] font-semibold mb-1">DIR trending toward threshold breach</p>
            <p className="font-mono text-xs text-[#A7B0C0] mb-3 leading-relaxed">
            ...
            <div className="flex items-center gap-5 mt-2 flex-wrap">
              <div className="flex items-center gap-1.5">
                <span className="inline-block h-2 w-2 rounded-full bg-[#6EA8FE]" />
                <span className="font-mono text-[10px] text-[#A7B0C0]">DIR over 8 quarters</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="inline-block h-px w-5 bg-red-500 opacity-60" style={{ borderTop: '2px dashed #FF5C6C', background: 'none' }} />
                <span className="font-mono text-[10px] text-[#A7B0C0]">0.80 four-fifths threshold</span>
              </div>
          ...
          <BentoCard>
            <div className="flex items-center gap-2 mb-2">
              <Search size={16} className="text-[#6EA8FE]" />
              <p className="font-mono text-xs font-semibold text-[#A7B0C0] uppercase tracking-wider">Proxy Detection</p>
            </div>
            <p className="font-mono text-sm text-[#F5F7FA] font-semibold mb-3">Reconstruction Risk Flagging</p>
              ...
              {proxyData.map(({ variable, score, fill, risk }) => (
                <div key={variable} className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-mono text-[10px] text-[#F5F7FA]">{variable}</span>
                    {risk && <span className="font-mono text-[9px] text-amber-400">⚠ risk</span>}
                  </div>
                  <span className="font-mono text-[10px] text-[#A7B0C0]">{score.toFixed(2)}</span>
                </div>
              ))}
            </div>
          ...
          <BentoCard>
            <div className="flex items-center gap-2 mb-2">
              <FileText size={16} className="text-[#F2C14E]" />
              <p className="font-mono text-xs font-semibold text-[#A7B0C0] uppercase tracking-wider">LDA Generator</p>
            </div>
            <p className="font-mono text-sm text-[#F5F7FA] font-semibold mb-3">Less Discriminatory Alternatives</p>
            ...
            <div className="mt-3 space-y-1.5">
              {ldaData.map(({ name, source, fill }) => (
                <div key={name} className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-[10px] text-[#A7B0C0] truncate">{name}</span>
                  <span className="font-mono text-[9px] text-[#A7B0C0]/60 ml-auto shrink-0">{source}</span>
                </div>
              ))}
            </div>

            <div className="mt-3 flex items-center gap-2">
              <span className="font-mono text-[10px] text-[#A7B0C0]">Avg approval ↑ with alt data:</span>
              <span className="font-mono text-xs text-[#6EA8FE] font-bold">
          ...
          <BentoCard className="md:col-span-2 flex flex-col justify-between">
            <div className="flex items-center gap-2 mb-3">
              <Shield size={16} className="text-[#6EA8FE]" />
              <p className="font-mono text-xs font-semibold text-[#A7B0C0] uppercase tracking-wider">Audit Performance</p>
            </div>
            <div className="flex items-end gap-8">
              <div>
                <p className="font-mono font-bold text-[#F2C14E] text-5xl mb-1">
                  <NumberTicker value={487} suffix="ms" delay={0.2} />
                </p>
                <p className="font-mono text-xs text-[#A7B0C0]">median end-to-end audit latency</p>
              </div>
              <div className="flex-1 h-px bg-[#1E2635]" />
              <div className="text-right">
                <p className="font-mono text-[#F5F7FA] font-bold text-2xl mb-1">5 sections</p>
                <p className="font-mono text-xs text-[#A7B0C0]">DIR · Proxy · Adverse Action · LDA · Remediation</p>
              </div>
            </div>
            <div className="mt-4 flex gap-2 flex-wrap">
              {['ECOA', 'Reg B', 'CFPB 2022-03', 'EEOC Four-Fifths', 'Fair Housing Act'].map((tag) => (
                <span key={tag} className="px-2 py-1 rounded-[4px] border border-[#1E2635] bg-[#06070A] font-mono text-[10px] text-[#A7B0C0]">{tag}</span>
              ))}
            </div>
          </BentoCard>
          ...
          <BentoCard>
            <div className="flex items-center gap-2 mb-3">
              <Lock size={16} className="text-[#6EA8FE]" />
              <p className="font-mono text-xs font-semibold text-[#A7B0C0] uppercase tracking-wider">Privacy Moat</p>
            </div>
            <p className="font-mono text-sm text-[#F5F7FA] font-semibold mb-2">Aggregate-only. No PII stored.</p>
            <div className="space-y-2">
              {['Supabase Row-Level Security', 'Aggregate results only', 'No protected-class PII retained'].map((item) => (
                <div key={item} className="flex items-center gap-2">
                  <span className="text-[#6EA8FE] text-xs">✓</span>
                  <span className="font-mono text-xs text-[#A7B0C0]">{item}</span>
                </div>
              ))}
            </div>
          </BentoCard>
