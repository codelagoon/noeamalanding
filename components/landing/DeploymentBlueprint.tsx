  ...
  return (
    <ResponsiveContainer width="100%" height={110}>
      ...
      >
          ...
          content={({ active, payload }) => {
            ...
            return (
              <div className="bg-[#06070A] border border-[#1E2635] rounded-lg px-3 py-2">
                <p className="font-mono text-xs text-[#F5F7FA] font-semibold">{row.label}</p>
                <p className="font-mono text-[10px] text-[#A7B0C0]">30-day sprint</p>
              </div>
            );
          }}
        />
        ...
        <Bar dataKey="p1" stackId="a" fill="#6EA8FE" radius={[4, 4, 4, 4]}>
          <LabelList dataKey="label" position="insideLeft" style={{ fontFamily: 'monospace', fontSize: 9, fill: '#06070A', fontWeight: 'bold' }} />
        </Bar>
        <Bar dataKey="p2" stackId="a" fill="#F2C14E" radius={[4, 4, 4, 4]}>
          <LabelList dataKey="label" position="insideLeft" style={{ fontFamily: 'monospace', fontSize: 9, fill: '#06070A', fontWeight: 'bold' }} />
        </Bar>
        <Bar dataKey="p3" stackId="a" fill="#6EA8FE" radius={[4, 4, 4, 4]}>
          <LabelList dataKey="label" position="insideLeft" style={{ fontFamily: 'monospace', fontSize: 9, fill: '#06070A', fontWeight: 'bold' }} />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  ...
  return (
    <section className="px-8 py-24 border-t border-[#1E2635]">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Gantt */}
        <div className="mb-10 p-5 rounded-xl border border-[#1E2635] bg-[#0A0D12]">
          <p className="font-mono text-[10px] uppercase tracking-wider text-[#A7B0C0] mb-4">Timeline</p>
          <GanttChart />
          <div className="flex gap-5 mt-3">
            {phases.map((p) => (
              <div key={p.phase} className="flex items-center gap-1.5">
                ...
                <span className="font-mono text-[9px] text-[#A7B0C0]">{p.phase}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {phases.map((p, i) => (
            <div key={p.phase} className="relative overflow-hidden rounded-xl border border-[#1E2635] bg-[#0A0D12] p-5">
              ...
              <div className="flex items-center gap-2 mb-3 mt-1">
                ...
                <span className="font-mono text-[10px] uppercase tracking-wider text-[#A7B0C0]">
                  Days {p.start + 1}–{p.start + p.duration}
                </span>
              </div>
              <h3 className="font-mono text-sm font-semibold text-[#F5F7FA] mb-4">{p.label}</h3>
              <ul className="space-y-2">
                {p.deliverables.map((d) => (
                  <li key={d} className="flex items-start gap-2">
                    <span className="mt-0.5 shrink-0 font-mono text-xs" style={{ color: p.color }}>→</span>
                    <span className="font-mono text-xs text-[#A7B0C0] leading-relaxed">{d}</span>
                  </li>
                ))}
              </ul>
              <div className="mt-4 pt-3 border-t border-[#1E2635]">
                <p className="font-mono text-[10px] text-[#A7B0C0]">
