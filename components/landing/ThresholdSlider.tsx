    ...
    <section className="px-8 py-24 border-t border-[#2A2A2A]">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Slider + live metrics */}
          <div className="rounded-xl border border-[#2A2A2A] bg-[#141414] p-6 flex flex-col gap-5">
            <div>
              <label className="font-mono text-xs font-semibold text-[#A0A0A0] uppercase tracking-wider block mb-3">
                Approval threshold
              </label>
              <div className="flex items-center justify-between mb-2">
                <span className="font-mono text-[10px] text-[#A0A0A0]">Permissive 0.30</span>
                <span className="font-mono text-[10px] text-[#A0A0A0]">Strict 0.80</span>
              </div>
              <input
                type="range"
                min={0.30}
                ...
                className="w-full accent-[#4A7C6F] cursor-pointer"
                style={{ accentColor: passes ? '#4A7C6F' : '#ef4444' }}
              />
              <p className="font-mono text-center text-lg font-bold mt-2" style={{ color: passes ? '#4A7C6F' : '#ef4444' }}>
                {threshold.toFixed(2)}
              </p>
            </div>
              ...
              {[
                {
                  ...
                  valueColor: current.dir < 0.8 ? 'text-red-400' : 'text-[#4A7C6F]',
                },
                {
                ...
                },
                {
                  ...
                  sub: 'All applicants',
                  valueColor: 'text-[#E8D5A3]',
                },
              ].map(({ label, value, sub, valueColor }) => (
                <div key={label} className="p-3 rounded-lg bg-[#0A0A0A] border border-[#2A2A2A]">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[10px] text-[#A0A0A0]">{label}</span>
                    <span className={`font-mono text-sm font-bold ${valueColor}`}>{value}</span>
                  </div>
                  <p className="font-mono text-[9px] text-[#A0A0A0] mt-0.5">{sub}</p>
                </div>
              ))}
            </div>
            ...
            <div className={`p-3 rounded-lg border font-mono text-xs leading-relaxed ${
              passes
                ? 'bg-[#4A7C6F]/10 border-[#4A7C6F]/30 text-[#4A7C6F]'
            ...
            <div className="p-3 rounded-lg bg-[#0A0A0A] border border-[#2A2A2A]">
              <p className="font-mono text-[10px] text-[#A0A0A0] mb-1 font-semibold">Rashomon Set</p>
              <p className="font-mono text-[10px] text-[#A0A0A0] leading-relaxed">
          ...
          <div className="lg:col-span-2 rounded-xl border border-[#2A2A2A] bg-[#141414] p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <ScatterChart margin={{ top: 8, right: 20, left: 0, bottom: 20 }}>
                ...
                <XAxis
                  ...
                  label={{ value: 'Model Accuracy (%)', position: 'insideBottom', offset: -12, fontFamily: 'monospace', fontSize: 9, fill: '#6b7280' }}
                />
                <YAxis
                  dataKey="y"
                  type="number"
                  ...
                  label={{ value: 'DIR', angle: -90, position: 'insideLeft', offset: 16, fontFamily: 'monospace', fontSize: 9, fill: '#6b7280' }}
                  width={40}
                />
                <Tooltip
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div className="bg-[#0A0A0A] border border-[#2A2A2A] rounded-lg px-3 py-2">
                        {d.lda && <p className="font-mono text-[10px] text-[#E8D5A3] font-semibold mb-1">{d.lda}</p>}
                        <p className="font-mono text-xs text-[#F5F5F5]">Accuracy: {d.x.toFixed(0)}%</p>
                        <p className="font-mono text-xs" style={{ color: d.y >= 0.8 ? '#4A7C6F' : '#ef4444' }}>
                          DIR: {d.y.toFixed(2)} {d.y >= 0.8 ? '✓' : '⚠'}
                        </p>
                        {d.threshold !== undefined && (
                          <p className="font-mono text-[10px] text-[#A0A0A0]">Threshold: {d.threshold.toFixed(2)}</p>
                        )}
                      </div>
                    );
                  }}
                ...
                {/* Baseline frontier */}
                <Scatter name="Baseline model" data={scatterData} fill="#374151" shape="circle" r={5} opacity={0.7} />

                {/* LDA frontier */}
                <Scatter name="LDA alternatives (Rashomon Set)" data={ldaScatter} fill="#E8D5A3" shape="diamond" r={5} opacity={0.85} />

                {/* Current selection */}
                <Scatter
                  name="Your current threshold"
                  data={currentPoint}
                  fill={passes ? '#4A7C6F' : '#ef4444'}
                  shape="circle"
                  r={9}
                />
              </ScatterChart>
            ...
            <div className="flex flex-wrap gap-5 mt-2">
              {[
                { color: '#374151', shape: '●', label: 'Baseline model (current data)' },
                { color: '#E8D5A3', shape: '◆', label: 'LDA alternatives (Rashomon Set)' },
                { color: passes ? '#4A7C6F' : '#ef4444', shape: '●', label: 'Your selected threshold' },
